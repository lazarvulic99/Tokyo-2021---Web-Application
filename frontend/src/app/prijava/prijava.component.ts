...
export class PrijavaComponent implements OnInit {
  ...
  prijava(){
    if(this.proveraPolja() == false){
      ...
      this.korisnikServis.prijavaNaSistem(this.kor_ime, this.lozinka, this.tip).subscribe((kor: Korisnik)=>{
        if(kor == null){
          this.korisnikServis.pronadjiKorisnika(this.kor_ime).subscribe((korisnik: Korisnik)=>{
            if(korisnik == null){
